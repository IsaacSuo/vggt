...
class VGGT(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024,
                 ...
                 enable_material=False, enable_light=False, enable_normal=False):
        super().__init__()
        ...
        if enable_material:
            print("🏗️  Initializing Material Head for PBR rendering...")
        ...
        # Normal prediction head for surface normals
        if enable_normal:
            print("📐 Initializing Normal Head for surface normal prediction...")
            self.normal_head = NormalHead(dim_in=2 * embed_dim, patch_size=patch_size)
        else:
    ...
    def forward(self, images: torch.Tensor, query_points: torch.Tensor = None):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # Normal prediction for surface normals
            if self.normal_head is not None:
                normals = self.normal_head(
                    aggregated_tokens_list, images=images, patch_start_idx=patch_start_idx
                )
