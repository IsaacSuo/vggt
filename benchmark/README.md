...
reconstruction against the mesh GT from `glossy-real-meshes-gt.zip` after
applying a precomputed scene-to-transform mapping checked into the repo. The
current checked-in mapping comes from manual Blender alignment, and the runtime
3D registration code is kept only as a fallback if a scene is missing from that
map. For object-level reconstruction filtering, the adapter rasterizes the
aligned mesh triangles into each selected frame to build benchmark masks, then
applies those masks before TSDF fusion. Because GlossyReal does not provide
